...
Notes by Driss
quick clarification about few important topics in the meeting ( Hopefully you can deliver that info to the relevant persons)
Data for testing: Data Team is actively working on providing test data that can be used on both SC1 and Client environments, they are the expert in the matter and we trust they will provide data as close to what the client is experiencing and based on that ( of course with collboration and validation for the parties who requested the data )
Reporting and transparency: our team is approaching a first release of reporting for test automation which being actively developed by one of our colleagues, this will allow SK to provide a report of all our tests runs on SC1
Test automation and client environment: For test automation to run they require expected input and expected output, as for now providing the expected input in client environment is not possible as we cannot pullote the client environment with test data, and using client data for testing purposes will hinder the ability of the tests to be independent from each other - the matter is being investigated but that only can be achieved when many criteria are met ( maturity, readiness, test data, ... )
